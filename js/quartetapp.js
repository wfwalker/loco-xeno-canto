var PlaceTimeBirdSongs = require('./placeTimeBirdSongs');
...
var gAudioContext = new webAudioAPI.AudioContext;
	...
	// $('#goSoundscape').click(function () {
 	...
 	// 	$("select#timeChooser option:selected").each(function() {
		...
		gBirds.getSightings(function(success, errorMessage) {
			if (success) {
				console.log('Choosing birds based on', gBirds.position, gBirds.description);

				for (var i = 0; i < gBirdSongPlayers.length; i++) {
					gBirdSongPlayers[i].chooseSightingAndPlayRandomSound(gBirds);
				}
			} else {
